...
cat(paste0("-----------------------------------------------------------------\n", 
           "Sequential reconciliation-combination (", type_arg, ")\nBf: ", 
...
# reconciliation
dir.create(file.path("fc", name, type_arg, folder_name), recursive = TRUE, 
           showWarnings = FALSE)
...
time_src <- matrix(NA, NROW(files), length(combs), dimnames = list(1:NROW(files),
                                                                   combs))
time_scr <- matrix(NA, NROW(files), length(combs), dimnames = list(1:NROW(files),
                                                                   combs))
...
for(j in 1:NROW(files)){
  ...
  for(comb in combs){
    start <- Sys.time()
    src <- FoCo2::cssrc(base = base, res = res, agg_mat = agg_mat, fc = type,
                       nn = info$nn, wnn = nn, comb = comb, shrink = FALSE, bounds = info$bounds)
    end <- Sys.time()
    time_src[j, comb] <- difftime(end, start, units = "secs")
    
    start <- Sys.time()
    scr <- FoCo2::csscr(base = base, res = res, agg_mat = agg_mat, fc = type,
                       nn = info$nn, wnn = nn, comb = comb, shrink = FALSE, bounds = info$bounds)
    end <- Sys.time()
    time_scr[j, comb] <- difftime(end, start, units = "secs")
    dimnames(scr) <- dimnames(src)
    
    reco <- recoinfo(src, verbose = FALSE)$reco
    ...
    reco <- lapply(1:length(reco), function(x){
      reshape2::melt(as.matrix(reco[[x]]), 
                     value.name = paste0(names(reco)[x], comb)) |>
        tibble() |>
        mutate(Var1 = abs(parse_number(as.character(Var1))))
    })
    reco <- Reduce(function(d1,d2) left_join(d1,d2,by=c("Var1", "Var2")), reco)
    if(is.null(fcdata)){
      fcdata <- reco
    }else{
    ...
    if(comb == combs[length(combs)]){
      ...
      base[[paste0(folder_name, "-",type_arg, "-base")]] <- rbind(recoinfo(scr, verbose = FALSE)$base)
      dimnames(base[[paste0(folder_name, "-",type_arg, "-base")]]) <- dimnames(base[[1]])
      base <- lapply(1:length(base), function(x){
        reshape2::melt(as.matrix(base[[x]]), 
                       value.name = names(base)[x]) |>
          tibble() |>
          mutate(Var1 = abs(parse_number(as.character(Var1))))
      })
      base <- Reduce(function(d1,d2) left_join(d1,d2,by=c("Var1", "Var2")), base)
...
save(time_src, time_scr,
     file = file.path("fc", name, "time", paste0("time_", type_arg, "_", folder_name, ".RData")))
