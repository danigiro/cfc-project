...
dir.create(
  file.path(
    paste0("./results/balanced", "_", structure, "_", cor_hat),
    paste0(
      "sim_p",
      p,
      "_nr",
...
with_progress(
  {
    ...
    df_full <- foreach(
    ...
    ) %dopar%
      {
          ...
          for (comb in combs_src) {
            for (app in c("sa", "var", "cov")) {
              ...
              if (is.null(src)) {
                src <- reshape2::melt(
                  reco,
                  value.name = paste0("src_", app, "-", comb)
                ) |>
                  as_tibble() |>
                  add_column(i = i, sim = id_sim, .before = 1) |>
                  rename(var = Var2) |>
              ...
              if (app == "sa") {
                uni <- as.list(attr(reco, "FoReco"))$reco
        ...
        df_reco <- full_join(
          full_join(df_occ, df_src, by = join_by(i, sim, var)),
          df_scr,
          by = join_by(i, sim, var)
        )
