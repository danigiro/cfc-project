...
dir.create(file.path(paste0("./results/unbalanced", "_", structure, "_", cor_hat), 
...
freq_mat <- matrix(c(0.59, 0.16,
                     0.41, 0.84), 2, 2)

unfreq_mat <- matrix(c(0.97, 0.31, 
                       0.03, 0.69), 2, 2)
...
with_progress({
  ...
  y <- foreach(id_sim = xs, .packages = c("tidyverse", "reshape2", "FoReco", "FoCo2")) %dopar% {
    if(type == "sett1"){
      # Setting 1
      mu_y <- 0
      mu_i <- rep(0, p)
    ...
    for(i in 1:sum(dim(agg_mat))){
      ...
      id <- which(rowSums(matres[-NROW(matres), ]*mat_na[-c(1:nr),i,])<=0)
      if(length(id)>0){
    ...
    for(i in 1:nh){
      ...
      for(comb in combs_occ){
        ...
        reco <- suppressWarnings(FoCo2::csocc(base = base_tmp, agg_mat = agg_mat, 
                                              res = res, comb = comb))
        if(is.null(occ)){
      ...
      for(comb in combs_scr){
        for(app in c("sa", "var", "cov")){
          reco <- suppressWarnings(FoCo2::csscr(base = base, res = res, agg_mat = agg_mat, fc = app,
                                                wnn = TRUE, comb = comb, shrink = FALSE))
          ...
          if(is.null(scr)){
            ...
            scr <- left_join(scr, reshape2::melt(reco, value.name = 
                                                   paste0(ifelse(comb == "none", "", "scr_"), 
                                                          app, "-", comb)) |>
                               as_tibble() |>
                               add_column(i = i, sim = id_sim, .before = 1) |>
                               rename(var = Var2) |>
...
}, enable = TRUE, delay_stdout=TRUE, delay_conditions="condition", cleanup = FALSE)
warnings()
